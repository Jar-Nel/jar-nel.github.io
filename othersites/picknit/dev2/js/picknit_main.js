...
window.addEventListener('click', e => {
    document.querySelectorAll('.divCategoryOptions').forEach(element => {
...
const formAddCategory = (categoryID) => {
    //addCategoryElement
    //  inputElement
    //  inputControlElement
    //    btnSaveElement
    ...
    //if there is a categoryID, the form is in edit mode.
    const oCat = oUser.getCategoryObject(categoryID, false);

    //Create a new element to contain the form
    const addCategoryElement = document.createElement('div');
    let inputTxt = oCat ? oCat.name : '';


    //Add an input field to get the category name
    const inputElement = document.createElement('input');
    inputElement.type = 'text';
    if (oCat) inputElement.value = oCat.name;
    ...
    btnSaveElement.addEventListener('click', (e) => {
        ...
        if (inputTxt.length < 1) {
            ...
            if (oCat) { //edit mode
                oCat.name = inputTxt;
                if (oUser.editCategory(categoryID, { name: inputTxt })) {
                    saveObj('oUser', oUser);
                    ...
                    document.getElementById('formCat_name_error').innerHTML = `Error saving category "${inputTxt}"`;
                }
            } else { //add mode
...
const categoryCardElement = (categoryID) => {
    ...
    */
    ...
    const btnDeleteCategoryElement = document.createElement('div');
    if (numPics === 0) {
        btnDeleteCategoryElement.textContent = 'Delete';
    ...
    const divCategoryOptions = document.createElement('div');
    ...
    const btnEditCategoryElement = document.createElement('div');
    btnEditCategoryElement.textContent = 'Rename';
    btnEditCategoryElement.title='Rename category.'
    btnEditCategoryElement.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
        divCategoryOptions.style.display = 'none';
        updateControlsPanel(formAddCategory(categoryID));
        updateContentPanel();
        setTimeout(() => {
            if (document.getElementById('formCat_name'))
                document.getElementById('formCat_name').select();
        }, 300);

    });

    const btnCategoryOptions = document.createElement('span');
    btnCategoryOptions.textContent = '[...]';

    divCategoryOptions.appendChild(btnEditCategoryElement);
    if (numPics===0)
        divCategoryOptions.appendChild(btnDeleteCategoryElement);
    divCategoryOptions.className = 'divCategoryOptions';
    divCategoryOptions.style.display = 'none';
    divCategoryOptions.style.flexFlow = 'column nowrap';
    divCategoryOptions.style.width = '120px';

    categoryCardElement.style.position = 'relative';
    

    btnCategoryOptions.addEventListener('click', e => {
        e.preventDefault();
        e.stopPropagation();
        divCategoryOptions.style.display = 'flex';
    });
    divCategoryOptions.addEventListener('mouseleave', e => {
        e.preventDefault();
        e.stopPropagation();
    ...
    categoryCardElement.appendChild(btnCategoryOptions);
    categoryCardElement.appendChild(divCategoryOptions);
    /*if (numPics === 0) {
    ...
    const categoryDetail = () => {
        ...
        category.pictures.forEach((picture) => {
            ...
            imageElement.addEventListener('click', (e) => {
                window.location = `viewImage.html#${picture.ID}`;
            })
